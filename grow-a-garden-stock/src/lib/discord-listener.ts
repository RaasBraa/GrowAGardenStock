...
import { parseStockEmbed, StockItem } from './stock-parser';
...
// We can add GEAR_CHANNEL_ID here later
  ...
  client.on(Events.MessageCreate, (message) => {
    ...
    if (stockType && message.author.id === TARGET_BOT_ID) {
      ...
      if (message.embeds.length > 0) {
        ...
        const parsedStock = parseStockEmbed(embed.toJSON());
        
        console.log(`Parsed [${stockType}] stock:`, parsedStock);
        ...
        allStockData[stockType.toLowerCase()] = parsedStock;
        allStockData.lastUpdated = new Date().toISOString();
        
        fs.writeFileSync(STOCK_DATA_PATH, JSON.stringify(allStockData, null, 2));
        console.log(`Successfully parsed and saved [${stockType}] stock to ${STOCK_DATA_PATH}`);
